...
- 明文节点、唯一的自动适配订阅 QR、原始订阅与 Base64 通用订阅
...
| `sudo asb -n` | 显示全部节点、所有订阅地址及唯一的自动适配订阅 QR |
...
`asb -n` 输出配置文件索引、全部原始节点链接，并且只输出一张自动适配订阅 QR。浏览器访问 `https://你的域名/你的UUID/` 可进入经典白底蓝字文件索引，再打开不同客户端配置：
...
终端二维码只为“自动适配订阅地址”生成。明文节点和 Clash、sing-box 等独立配置地址不再生成二维码，避免重复输出并减少误扫。
