...
终端输出按“紫色品牌标题 → 蓝色分区标题 → 青色键名/序号 → 白色内容”建立固定层级，并使用绿、黄、红分别表示成功、警告和错误。菜单、诊断、节点订阅与表格统一采用内容块布局：区块之间保留一行，区块内部保持紧凑，避免连续空行。命令提示使用紫色箭头，链接带下划线；重定向输出、`TERM=dumb` 或设置 `NO_COLOR=1` 时自动关闭全部颜色和文本装饰。
