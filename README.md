...
| `sudo asb -x` | 执行运行诊断、WS 检查并显示最近日志 |
...
终端输出采用紧凑分区布局：标题固定为 `Argo-Singbox  v版本号`，下方使用随终端宽度限制在 56 到 76 列之间的单行分隔线；区块标题不再加方括号；菜单项使用 `序号  名称  命令`；状态只使用 `[OK] / [WARN] / [ERR] / [INFO] / [SKIP]` 前缀。品牌与结构使用亮蓝，版本使用少量亮紫，分区和键名使用亮青，菜单序号使用亮黄，正文使用亮白，成功/警告/错误分别使用绿/黄/红。长订阅 URL 不再使用下划线，所有交互提示统一以 `>` 开头。重定向输出、`TERM=dumb` 或设置 `NO_COLOR=1` 时自动关闭全部颜色和文本装饰。

## 诊断、备份与恢复

- `asb -x`：检查配置与 Token 同步、三个服务、全部动态监听端口、每条公网 WS 路径、核心版本和 WARP（启用时）。正常通过时只输出诊断结果；存在失败时再输出最近 30 条项目日志。
...
`asb -v` 会分别显示 cloudflared 与 sing-box 的本地、目标版本，并分别询问是否更新。只下载、备份、替换和重启用户确认更新的核心；下载文件会校验 SHA256/可执行性，sing-box 还会执行配置检查。验证失败时只回滚本次选择的核心。
...
`asb -n` 输出网页订阅面板、全部订阅链接和明文节点，不在终端输出 QR 二维码。浏览器访问 `https://你的域名/你的UUID/` 可进入清晰卡片式订阅面板，面板标题显示 `Argo-Singbox 订阅面板`，包含自动适配订阅 QR，并可打开不同客户端订阅：
